...
{
	cout<<idnum<<setw(6)<<" "<<name<<setw(10)<<cpp<<setw(4)<<cloud_computing<<setw(4)<<drone<<setw(4)
		<<hpc<<setw(4)<<evs<<setw(8)<<per<<setw(6)<<grade<<endl;
}
...
void delete_student(int n)
{
	student st;
	ifstream inFile;
	inFile.open("studdent.dat",ios::binary);
	if(!inFile)
	{
		cout<<"File could not be open !! Press any key...";
		cin.ignore();
